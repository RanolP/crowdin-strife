...
impl E2K {
    pub async fn execute<'a>(self, asset_store: &AssetStore<'a>) -> eyre::Result<String> {
        let en_us = read_lang_file(&asset_store.read_asset("lang/dungeons/en_us.json").await?)?;
        let ko_kr = read_lang_file(&asset_store.read_asset("lang/dungeons/ko_kr.json").await?)?;
